...
namespace DSIGE.Dato
{
    public class Cls_Dato_VerificacionFotos
    {
        ...
        public string Capa_Dato_EnviarFotosSinObservacion(string lecturas, int id_usuario)
        {
            string Resultado = "";
            try
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_I_U_VERIFICAR_FOTOS_FOTOS_SIN_OBSERVACION_V2", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@lecturas", SqlDbType.VarChar).Value = lecturas;
